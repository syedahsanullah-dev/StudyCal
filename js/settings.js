...
const SETTINGS = (() => {
  ...
  function _populateSubjectsTab() {
    ...
    s.subjects.forEach((sub, idx) => {
      ...
      row.innerHTML = `
        ...
        ${SESSIONS.renderReadOnly(sub.onlineSessions)} 
      `;
  ...
  function _readSubjectsTab() {
    ...
    rows.forEach(row => {
      ...
      updated.push({
        ...
        lecsPerDay: lpd,
        days:       days.length > 0 ? days : orig.days,
        onlineSessions: orig.onlineSessions || [] // Ensure sessions are kept when saving!
      });
    });
  ...
  function _initAddSubject() {
    ...
    function openModal() {
      ...
      // INITIALIZE SESSIONS SYSTEM FOR NEW SUBJECT
      SESSIONS.init([]); 
      SESSIONS.bindAddBtn();
    ...
    confirmBtn?.addEventListener("click", () => {
      ...
      const newSub = {
        ...
        lecsPerDay: null,
        active:     true,
        onlineSessions: SESSIONS.getSessions() // GRAB THE SESSIONS ADDED!
      };
